...
public class DBHelper extends SQLiteOpenHelper {
    ...
    private static final String DBNAME = "CM_CACHE.db";

    private static final String TABLE_CACHE_URL_DATA = "cacheUrlData";
    private static final String TABLE_CUD_FILED_URL = "url";
    private static final String TABLE_CUD_FILED_CACHEDATA = "cacheData";
    private static final String TABLE_CUD_FILED_CREATETIME = "createTime";
    ...
    public String[] getURLData(String url) {
        ...
        try {
            ...
            cursor = db.rawQuery("select cacheData,createTime from " + TABLE_CACHE_URL_DATA + " where " + TABLE_CUD_FILED_URL + " = ?", new String[]{url});
    ...
    public synchronized void addOrUpdateURLData(String url, String jsonData) {
        try {
            ...
            if (isExists) {
                db.execSQL("update " + TABLE_CACHE_URL_DATA + " set " + TABLE_CUD_FILED_CACHEDATA + "=?, " + TABLE_CUD_FILED_CREATETIME +
                        " = ? where " + TABLE_CUD_FILED_URL + " =? ", new String[]{jsonData, jsonData, sdf.format(new Date()), url});
            } else {
                db.execSQL("insert into " + TABLE_CACHE_URL_DATA + " (" + TABLE_CUD_FILED_URL + "," + TABLE_CUD_FILED_CACHEDATA + "," +
                        TABLE_CUD_FILED_CREATETIME + ") values(?, ?, ?)", new String[]{url, jsonData, sdf.format(new Date())});
            }
        } catch (Exception e) {
            e.printStackTrace();
            CMLog.LogD(" url r" + url);
        } finally {
//			db.close();
        }
    }
    ...
    public boolean deleteURLData() {
        try {
            db = getWritableDatabase(); // 获得数据库写对象\
            return db.delete(TABLE_CACHE_URL_DATA, null, null) > 0;
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            //		db.close();
    ...
    public boolean deleteURLData(String url) {
        try {
            db = getWritableDatabase(); // 获得数据库写对象\
            return db.delete(TABLE_CACHE_URL_DATA, TABLE_CUD_FILED_URL + " like ?", new String[]{"%" + url + "%"}) > 0;
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
//			db.close();
    ...
    private synchronized boolean checkURLData(String url) {
        ...
        try {
            ...
            cursor = db.rawQuery("select cacheData from " + TABLE_CACHE_URL_DATA + " where " + TABLE_CUD_FILED_URL + " = ? ", new String[]{url});
            while (cursor.moveToNext()) {
                cursor.close();
                return true;
            }
...
//	}
    ...
    public void onCreate(SQLiteDatabase db) {
        CMLog.LogD("Sql onCreate ");
        db.execSQL("create table if not exists " + TABLE_CACHE_URL_DATA + "(" +
                TABLE_CUD_FILED_URL + " varchar primary key, " +
                TABLE_CUD_FILED_CACHEDATA + " varchar, " +
                TABLE_CUD_FILED_CREATETIME + " varchar)");
    }
